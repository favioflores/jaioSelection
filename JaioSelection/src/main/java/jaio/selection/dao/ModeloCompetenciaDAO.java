...
public class ModeloCompetenciaDAO extends HibernateUtil implements Serializable {
    ...
    public List obtenerCompetenciasDeDroppBaterias(StringBuilder sb) {
        iniciaSession();
        try {
            Query query = session.createSQLQuery("select distinct(mc.nombre),mc.id, mc.color,mec.modelo_evaluacion_id from modelo_evaluacion_x_competencia mec"
                    + " join modelo_evaluacion me on me.id = mec.modelo_evaluacion_id"
                    + " join modelo_competencia mc on mc.id = mec.modelo_competencia_id"
                    + " where mec.modelo_evaluacion_id in(" + sb + ")");
            return query.list();
        } catch (Exception e) {
            manejaException(log, e);
        } finally {
